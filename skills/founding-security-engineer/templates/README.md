...
| Recommendation drift | The moment a decision changes how an already-flagged item will be handled - deferred, rerouted, accepted, downgraded - rewrite that item's own recommended-action or next-step field to say so: the original recommendation, what was actually decided, and that it was deliberate. A `DECISION-LOG.md` entry alone is not enough, because a future session reads the flagged item literally - a `RISK-REGISTER.md` row, a `90-DAY-PLAN.md` step, a `CONTEXT-STACK.md` frame - and re-recommends the original action if the item's own text still states it unqualified. Caught live in the field: a critical risk's recommendation read "message the CTO today, don't wait" after the human had already chosen to route it through a scheduled 1:1 instead; a fresh session read the stale text and re-recommended sending it as a standalone message, effectively re-litigating a settled decision. |
